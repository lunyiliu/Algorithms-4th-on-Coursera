...
public class SeamCarver_origin {
    ...
    public SeamCarver_origin(Picture picture) {
        if (picture == null) {
    ...
    public int[] findHorizontalSeam() {
        ...
        for (int i = 0; i < height(); i++) {
            ...
            int[][][] edgetos = new int[width()][height()][2];
            ...
            if (seam[i] != height - 2) {
                new_energy[i][seam[i]] = Math
                        .sqrt(Math.pow(Right_down.getRed() - Left_down.getRed(), 2) + Math
                                ...
                                .pow(Right_down.getGreen() - Left_down.getGreen(), 2) + Math.pow(
                                Up_1.getRed()
    ...
    public void removeVerticalSeam(int[] seam) {
        ...
        Color[][] new_pixels = new Color[height][width - 1];
        ...
        for (int i = 0; i < height; i++) {
            ...
            if (seam[i] != 1) {
                Up_2 = graph.get(seam[i] - 2, i);
            }
            Color Up_1 = graph.get(seam[i] - 1, i);
            Color Right_up = graph.get(seam[i] - 1, i + 1);
            Color Left_up = graph.get(seam[i], i - 1);
            Color Right_down = graph.get(seam[i] + 1, i + 1);
            Color Left_down = graph.get(seam[i] + 1, i - 1);
            Color Down_1 = graph.get(seam[i] + 1, i);
            if (seam[i] != height - 2) {
                Down_2 = graph.get(seam[i] + 2, i);
            }
            if (seam[i] != 1) {
                new_energy[i][seam[i] - 1] = Math
                        .sqrt(Math.pow(Right_up.getRed() - Left_up.getRed(), 2) + Math
        ...
        for (int i = 0; i < width - 1; i++) {
            for (int j = 0; j < height; j++) {
                new_picture.set(i, j, new_pixels[j][i]);
            }
        }
        */
        double[][] trans_energy = new double[height()][width()];
        ...
        height = width;
        width = tmp;
        trans_energy = new double[width][height];
...
private class Node implements Comparable<Node> {
    ...
    public Node(int x_, int y_, double dist_) {
        x = x_;
        y = y_;
        dist = dist_;
    }
    ...
    int[] SP = new int[width()];
        for(int i=0;i<height();i++){
        int[][]touched_and_in_PQ=new int[width()][height()];
        MinPQ<Node> to_touch_vertice=new MinPQ<Node>();
        to_touch_vertice.insert(new Node(0,i,0));
        double[][]dists
        =new double[width()][height()];// for every vetex in left column, there is a dist[][]
        int[][][]edgetos=new int[width()][height()][2];
        for(int j=0;j<width();j++){
        ...
        if(j!=0){
        dists[j][k]=Double.POSITIVE_INFINITY;
        touched_and_in_PQ[j][k]=0;
        }
        else{
        dists[j][k]=0;
        touched_and_in_PQ[j][k]=2;
        }
        edgetos[j][k][0]=-1;
        edgetos[j][k][1]=-1;
        ...
        //StdOut.println("i=" + i);
        while(!to_touch_vertice.isEmpty()){
        // StdOut.println("   j=" + j);
                /*
                Iterator iter = to_touch_vertice.iterator();
                while (iter.hasNext()) {
                    Node next = (Node) iter.next();
                    String str = next.x + "," + next.y + "," + next.dist;
                    StdOut.println("        " + str);
                }
                */
        Node current=to_touch_vertice.delMin();
        //StdOut.println("   current.x: " + current.x);
        // StdOut.println("   current.y: " + current.y);
        touched_and_in_PQ[current.x][current.y]=2;
        int j=current.x+1;

        for(int k=current.y-1;k<current.y+2;k++){
        // StdOut.println("      k=" + k);
        if(k>=0&&k<height()&&j>=0&&j<width()){
        //StdOut.println("width" + width());
        if(k==current.y-1){
        if(dists[current.x][current.y]+energy[j][k]<dists[j][k]){
        dists[j][k]=dists[current.x][current.y]+energy[j][k];
        edgetos[j][k][0]=j-1;
        edgetos[j][k][1]=k+1;
        }
        if(touched_and_in_PQ[j][k]==0){
        // StdOut.println("current.j: " + j);
        //  StdOut.println("current.k: " + k);
        to_touch_vertice.insert(new Node(j,k,dists[j][k]));
        touched_and_in_PQ[j][k]=1;//inside PQ
        }
        }
